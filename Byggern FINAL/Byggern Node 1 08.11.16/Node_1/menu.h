...
// Enums:
typedef enum {
	mainID = 40, newGameID, HighScoresID, OptionsID, currentMenuID, easyGameID, normalGameID, hardGameID, CreditsID, ControllerID, SpeedControllerID,
	PositionControllerID, MarioID, SolenoidModeID, InvertedModeID, SolenoidAutoID, SolenoidSemiID, SolenoidToggleAutoID, NonInvertedID, InvertedID, resetHighscoreID
} menuID;
...
typedef struct startgame_settings{
    ...
    // Chosen difficulty:
	uint8_t difficulty;
    // Chosen solenoid mode:
	uint8_t solenoid;
    // BOOL value determining inverted mode:
	uint8_t inverted;
} game_settings;
