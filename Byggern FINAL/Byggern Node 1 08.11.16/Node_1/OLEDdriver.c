...
/*! \brief OLED print mario.
 *  Prints picture of mario to OLED.
 *
 *  \param void
 *
 *  \return void
...
void OLED_print_mario(){
	int temp = col;
		for (uint8_t k = 0; k < 4; k+=2){

			for (uint8_t j = 0; j < 2; j++){
				for (uint8_t i = 0; i < FONTSIZE; i++){
					OLED_write_data(pgm_read_byte(&mario[j+k][i]));
				}
			}
			OLED_goto_page(page+1);
			OLED_goto_column(temp);
		}
		OLED_home();
}


/*! \brief OLED print pipe.
 *  Prints picture of pipe to OLED.
 *
 *  \param void
 *
 *  \return void
...
void OLED_print_pipe(){
	int temp = col;
		for (uint8_t k = 0; k < 4; k+=2){

			for (uint8_t j = 0; j < 2; j++){
				for (uint8_t i = 0; i < FONTSIZE; i++){
					OLED_write_data(pgm_read_byte(&pipe[j+k][i]));
				}
			}
			OLED_goto_page(page+1);
			OLED_goto_column(temp);
...
/*! \brief OLED print monster.
 *  Prints picture of monster to OLED.
 *
 *  \param void
 *
 *  \return void
...
void OLED_print_monster(){
	int temp = col;
		for (uint8_t k = 0; k < 4; k+=2){

			for (uint8_t j = 0; j < 2; j++){
				for (uint8_t i = 0; i < FONTSIZE; i++){
					OLED_write_data(pgm_read_byte(&monster[j+k][i]));
				}
			}
			OLED_goto_page(page+1);
			OLED_goto_column(temp);
...
void OLED_clear_mario(){
	OLED_fill_char();
	OLED_go_to_pos(page, col+8);
	OLED_fill_char();
	OLED_go_to_pos(page+1, col-8);
	OLED_fill_char();
	OLED_go_to_pos(page, col+8);
	OLED_fill_char();
}